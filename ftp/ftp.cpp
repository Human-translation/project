...
int FTPClient::socket_connect(const char *host, int port)
{
	...
	{
		printf("socket or setsockopt create failed\n");
		return -1;
	}
	//设置接收和发送超时
	...
	if (!server)
	{
		printf("gethostbyname conversion failed\n");
		return -1;
	}
...
int FTPClient::ftp_retrfile(int c_sock, char *s, char *d, unsigned long long *stor_size, int *stop)
{
	...
	if (d_sock == -1)
	{
		printf("ftp pasv connect failed\n");
		close(handle);
		return -1;
	}
